@props(['id' => null, 'title', 'category', 'description', 'link' => '#', 'number' => '01', 'image' => null, 'github_link' => null, 'views' => 0])
...
<div x-data="{ views: {{ (int)$views }}, projectId: @js($id) }"
     ...
     <!-- Right: Action Links & Views -->
     <div class="flex flex-col md:items-end justify-center gap-2 flex-shrink-0 md:w-48">
         <div class="flex items-center gap-4 text-xs md:text-sm font-semibold">
             @if($category === 'Web Dev')
                 <a href="{{ $link }}" target="_blank" @click.stop="if(projectId){ fetch('/api/projects/' + projectId + '/view', { method: 'POST' }); views++; }" class="text-white hover:text-blue-500 flex items-center gap-1.5 transition-colors">
                     <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                     Website
                 </a>
                 @if($github_link)
                 <a href="{{ $github_link }}" target="_blank" @click.stop="if(projectId){ fetch('/api/projects/' + projectId + '/view', { method: 'POST' }); views++; }" class="text-gray-400 hover:text-white flex items-center gap-1.5 transition-colors">
         ...
         <div class="text-[10px] md:text-xs text-gray-500 flex items-center gap-1.5 font-medium">
             <svg class="w-3.5 h-3.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
             <span x-text="views.toLocaleString('en-US')"></span> views
         </div>
     </div>
</div>
