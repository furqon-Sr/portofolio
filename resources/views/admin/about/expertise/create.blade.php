...
@section('title', 'Tambah Expertise Logo')
@section('page-title', 'Tambah Expertise Logo')
...
<div class="max-w-3xl mx-auto space-y-6 animate-fade-in">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h3 class="text-lg font-bold text-white tracking-tight">Tambah Keahlian / Tech Logo Baru</h3>
            <p class="text-xs text-gray-500 mt-0.5">Tambahkan logo teknologi baru ke grid keahlian.</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-[#111111] rounded-2xl border border-white/5 p-6 md:p-8 shadow-xl">
        <form method="POST" action="{{ route('admin.about.expertise.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" id="name" required placeholder="Contoh: Laravel" value="{{ old('name') }}" 
                ...
                <div class="space-y-2">
                    ...
                    <input type="url" name="url" id="url" placeholder="Contoh: https://laravel.com" value="{{ old('url') }}" 
                           class="w-full bg-white/[0.02] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all">
                    @error('url')
                        <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                    @enderror
            ...
            <div class="space-y-4 p-5 bg-black/30 rounded-2xl border border-white/5" x-data="{ uploadType: 'file' }">
                ...
                <div x-show="uploadType === 'file'" class="space-y-2">
                    <label for="logo_file" class="block text-xs font-bold uppercase tracking-wider text-gray-400">File Gambar Logo</label>
                    <input type="file" name="logo_file" id="logo_file" accept="image/*"
                           class="w-full bg-white/[0.02] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all">
                    <p class="text-[10px] text-gray-500 mt-1">Gunakan gambar berasio 1:1 (Square). Maksimal ukuran file 2MB.</p>
                    @error('logo_file')
                        <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div x-show="uploadType === 'url'" class="space-y-2" style="display: none;">
                    <label for="logo_url" class="block text-xs font-bold uppercase tracking-wider text-gray-400">URL Gambar Logo</label>
                    <input type="url" name="logo_url" id="logo_url" placeholder="Contoh: https://logo.com/react.png" value="{{ old('logo_url') }}"
                           class="w-full bg-white/[0.02] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all">
                    @error('logo_url')
                        <p class="text-xs text-red-500 font-medium">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-white/5 pt-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="bg_class" id="bg_class" required placeholder="bg-white atau bg-[#FF2D20]/10" value="{{ old('bg_class', 'bg-white') }}" 
                           class="w-full bg-white/[0.02] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="hover_class" id="hover_class" required placeholder="hover:border-blue-500 atau hover:border-[#FF2D20]" value="{{ old('hover_class', 'hover:border-blue-500') }}" 
                           class="w-full bg-white/[0.02] border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all">
